...
def _select_best_recorded_candidate(state: Dict[str, Any]) -> str:
    expected_type = state.get("question_plan", {}).get("answer_type", "other")
    candidates = [
        _normalize_answer_to_type(str(item.get("text", "")), expected_type)
    ...
    if not candidates:
        return ""
    best = max(candidates, key=lambda item: _candidate_evidence_score(item, state))
    if _candidate_evidence_score(best, state) < 0:
        return ""
    return best
